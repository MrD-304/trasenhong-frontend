(function () {
  ...
  const EMAILJS_TEMPLATE_ID = "template_gv0erpa";  // template mới
  ...
  form.addEventListener("submit", async function (e) {
    ...
    btn.disabled = true;
    btn.innerHTML = '<span class="material-symbols-outlined" aria-hidden="true">hourglass_empty</span> Đang gửi...';
    const subjectVal = form.querySelector("[name=subject]")?.value || "other";
    const templateParams = {
      from_name:     form.querySelector("[name=name]").value.trim(),
      from_phone:    form.querySelector("[name=phone]").value.trim(),  // khớp {{from_phone}}
      from_email:    form.querySelector("[name=email]").value.trim(),
      subject_label: subjectMap[subjectVal] || subjectVal,             // khớp {{subject_label}}
    ...
    try {
      ...
      btn.disabled = false;
      btn.innerHTML = '<span class="material-symbols-outlined" aria-hidden="true">send</span> Gửi Tin Nhắn';
      alert("Gửi thất bại, vui lòng thử lại hoặc liên hệ trực tiếp qua hotline.");
    }
  });
})();
